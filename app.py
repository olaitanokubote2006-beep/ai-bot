...
# 🔒 DEFENSIVE: Check required env vars BEFORE initializing anything
REQUIRED_VARS = ["SUPABASE_URL", "SUPABASE_KEY", "TELEGRAM_BOT_TOKEN"]
missing_vars = [v for v in REQUIRED_VARS if not os.environ.get(v)]

if missing_vars:
    print(f"❌ FATAL: Missing environment variables: {missing_vars}", file=sys.stderr)
    print("💡 Add them in Railway → Variables tab → Redeploy", file=sys.stderr)
    
    @app.route('/')
    def setup_error():
        return f"""
        <div style="font-family:sans-serif;max-width:600px;margin:40px auto;padding:20px;border:2px solid #ef4444;border-radius:8px">
            <h1 style="color:#ef4444">⚠️ Deboo Setup Incomplete</h1>
            <p><b>Missing environment variables:</b></p>
            <ul>{"".join(f"<li>{v}</li>" for v in missing_vars)}</ul>
            <p><b>Fix:</b> Add these in your Railway dashboard → Variables tab → Save → Redeploy</p>
            <p style="font-size:12px;color:#666">Error code: ENV_MISSING_{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}</p>
        </div>
        """, 503
    
    @app.route('/health')
    def health_error():
        return {"status": "error", "message": "Missing environment variables", "missing": missing_vars}, 503
    
    # Skip bot initialization — app will show error page instead of crashing
    sb = None
    PAY_KEY = None
    ...
    VOICE_NOTES = {
        "greet": "https://www.soundjay.com/buttons/button-1.mp3",
    ...
    def generate_paystack_link(amount, email, ref, callback):
        try:
            r = requests.post(
                ...
                json={"email": email, "amount": int(amount) * 100, "currency": "NGN", "reference": ref, "callback_url": callback}
            )
    ...
    def ai_logic(txt, st, ctx, biz, lang):
        ...
        if st == "PENDING_FEE":
            fee = calc_fee(txt)
            ctx["delivery_fee"] = fee; ctx["total"] = ctx["amount"] + fee; ctx["location"] = txt
    ...
    def send_tg_voice(cid, audio_url, token, caption=""):
        try:
            requests.post("https://api.telegram.org/bot" + token + "/sendVoice", json={"chat_id": cid, "voice": audio_url, "caption": caption, "parse_mode": "HTML"})
        except Exception as e:
...
if __name__ == '__main__':
    if missing_vars:
        print("⚠️ Running in setup-error mode — add env vars in Railway")
    else:
        print("🌐 Loading active bots...")
        load_all_bots()
